...
func setup(character : Character) -> void:
	...
	for attribute_name : String in attribute_names:
		...
		if ability_component.has_attribute(attribute_name):
			var attribute: AbilityAttribute = ability_component.get_attribute(attribute_name)
			var attribute_value := ability_component.get_attribute_value(attribute_name)
