...
func _update_ui_scale() -> void:
    ...
    match scale_mode:
        ...
        ScaleMode.STRETCH:
            current_scale = Vector2(scale_x, scale_y)
        ScaleMode.PIXEL:
...
func _on_viewport_size_changed() -> void:
    actual_resolution = get_viewport().get_visible_rect().size
...
func get_adapted_size(original_size: Vector2) -> Vector2:
    if current_scale is float:
        ...
        return Vector2(
            original_size.x * current_scale.x,
            original_size.y * current_scale.y
        )
