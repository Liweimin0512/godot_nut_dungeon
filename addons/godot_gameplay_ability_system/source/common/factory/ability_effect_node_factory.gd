extends RefCounted
class_name AbilityEffectNodeFactory
...
## 缓存已加载的JSON配置
static var ability_effect_node_cache: Dictionary[String, AbilityEffectNode] = {}

## 注册新的节点类型
static func register_node_type(type_name: String, script_path: String) -> void:
...
## 注销节点类型
static func unregister_node_type(type_name: String) -> void:
    if _node_type_map.has(type_name):
...
## 从JSON配置创建效果节点树
static func create_from_config(config: Dictionary) -> AbilityEffectNode:
...
## 从JSON文件创建效果节点树
static func create_from_json(json_path: String) -> AbilityEffectNode:
    # 先检查缓存
    if ability_effect_node_cache.has(json_path):
        return ability_effect_node_cache[json_path]
    var config = JsonLoader.load_json(json_path)
    var node = create_from_config(config)
    ability_effect_node_cache[json_path] = node
    return node
